...
if "__main__" == __name__:
    ...
    try:
        ...
        order = trader.order_manager.create_market_order(Stock("MSFT"), 10, order_type=OrderType.BUY)
        order.on_fill = fill_alert
        order.on_cancel = cancel_alert
        order.place()

#        m,p,s = trader.order_manager.create_bracket_order(Stock("AAPL"), 10, 325, 300)
        # order.on_fill = fill_alert
        # order.on_cancel = cancel_alert
        # order.place()
        m.place()
